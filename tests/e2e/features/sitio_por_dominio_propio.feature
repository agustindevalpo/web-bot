Feature: Sitio servido por dominio propio
  ...
  # El Worker de Cloudflare agrega X-Forwarded-Host con el dominio original;
  # en local se simula esa cabecera directamente (WORKER_SHARED_SECRET vacío).

  Scenario: Sitio con dominio propio se sirve por X-Forwarded-Host
    Given un sitio activo con subdominio "e2e-dominio" y dominio propio "e2e-dominio.webbot.test"
    When visito la raíz con la cabecera X-Forwarded-Host "e2e-dominio.webbot.test"
  ...
  Scenario: Dominio propio desconocido no se sirve
    When visito la raíz con la cabecera X-Forwarded-Host "e2e-no-existe-jamas.webbot.test"
