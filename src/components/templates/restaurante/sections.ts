...
// RESTAURANTE sí incluye galería (Decisión D6: "warm gallery strip"),
...
export type MenuProps = { etiqueta: string; items: string[] }
...
export function buildMenu(config: SiteConfigDTO): MenuProps | null {
  const crudo = config.servicios ?? []
  if (crudo.length === 0) return null
  return { etiqueta: ETIQUETA_MENU, items: crudo.map((item) => nombreDeServicio(item) ?? '') }
}
