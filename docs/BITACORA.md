...
MERCADOPAGO_LINK_URL=
```

Pendiente manual de Agustín: agregar la línea a `.env.example` (la tooling de esta sesión no puede tocar dotfiles) y cargarla en Railway con `railway variables set MERCADOPAGO_LINK_URL=https://mpago.la/...`. Se lee en el servidor (no es `NEXT_PUBLIC_*`), así que basta un redeploy.
...
- **`MERCADOPAGO_LINK_URL` en `.env.example`/Railway sigue pendiente** — ver la sección anterior; tarea manual de Agustín, no bloquea este slice porque el link ya cae a `/login` sin la variable.
...
1. **(Manual, Agustín)** Agregar `MERCADOPAGO_LINK_URL` a `.env.example` y cargarla en Railway — la tooling de esta sesión no puede tocar dotfiles.
